...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // make sure you comment it out when you add your own code -- otherwise e.g. you might integrate yaw twice
	...
	// non-linear complemetary filter. 
	// A quaternion for the euler angles
	Quaternion<float> qt, dq, qtPredicted;
	  
	qt = qt.FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
	qtPredicted = dq.IntegrateBodyRate(gyro, dtIMU) * qt;

	float predictedPitch = qtPredicted.Pitch();
	float predictedRoll = qtPredicted.Roll();
	ekfState(6) = qtPredicted.Yaw();
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(5) = curState(5) + (attitude.Rotate_BtoI(accel).z * dt - static_cast<float>(CONST_GRAVITY * dt));
